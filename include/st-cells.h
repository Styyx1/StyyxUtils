...
struct CellUtil
{
    ...
    static bool IsDungeon(const RE::TESObjectCELL* a_cell)
    {
        ...
        if (const auto loc = a_cell->GetLocation();
            loc && loc->HasAnyKeywordByEditorID(is_a_dungeon) && a_cell->IsInteriorCell())
        {
            return true;
        }
        return false;
    ...
    static bool IsJail(RE::TESObjectCELL* a_cell)
    {
        ...
        if (const auto key = RE::TESForm::LookupByEditorID<RE::BGSKeyword>("LocTypeJail");
            key && loc && loc->HasKeyword(key))
        {
            return true;
        }
        return false;
