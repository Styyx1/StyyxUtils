...
{
    explicit ScopedSharedLock(std::shared_mutex& mtx)
        : lock(mtx)
    {
    }
...
{
    explicit ScopedUniqueLock(std::shared_mutex& mtx)
        : lock(mtx)
    {
    }
    void unlock() { lock.unlock(); }
...
struct ScopedSharedLock
{
    explicit ScopedSharedLock(std::shared_timed_mutex& mtx)
...
struct ScopedUniqueLock
{
    explicit ScopedUniqueLock(std::shared_timed_mutex& mtx)
        : lock(mtx, std::defer_lock)
    {
        if (!lock.try_lock_for(std::chrono::seconds(5)))
        {
