...
struct MenuUtil {
  ...
  IsAnyOfMenuOpen(const std::vector<std::string> &a_menuNames) {
  ...
  static void VisitMenuMembersRec(const char *a_path, RE::GFxValue &a_obj,
    ...
    struct MemberVisitor : RE::GFxValue::ObjectVisitor {
      ...
      void Visit(const char *a_name, const RE::GFxValue &a_val) override {
        ...
        case RE::GFxValue::ValueType::kDisplayObject: {
          ...
          VisitMenuMembersRec(full_path.c_str(), val_copy, depth + 1,
                              max_depth);
          break;
        }
        default:
